...
const PurchaseUI = {
  ...
  _updateBatchInfo() {
    ...
    if (!activeCount && (!diag.mode || diag.mode === 'none')) {
      el.innerHTML = '<span style="color:#ef4444">（⚠️ 未记录本次上传 SKU，当前显示' + scopeLabel + '全部 ' + (diag.myTotal || '?') + ' 条）</span>';
      return;
    }
    ...
    el.innerHTML = `<span style="color:#22c55e">（本次上传：${fn}${ts} · ${activeCount} 条；${scopeLabel}共 ${myTotal} 条，当前显示 ${finalCount} 条）</span>`;
  },
  ...
  getDeliveryBasedData() {
    ...
    if (!this.isAdmin) {
      ...
      this._filterDiag = {
        buyer: this.userName,
        isCategoryManager: this.isCategoryManager,
      ...
      console.log(`[Purchase] ${this.userName} ${this.isCategoryManager ? '品类负责' : '采购员'}过滤(showAll=${!!showAll}, mode=${filterMode}, active=${activeKeys.length}): ${before} -> ${rows.length}`);
    }
